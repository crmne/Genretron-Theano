...
def shared_dataset(data_x, data_y, borrow):
    ...
    shared_x = theano.shared(value=data_x, borrow=borrow)
    shared_y = theano.shared(value=data_y, borrow=borrow)
    return shared_x, shared_y


def prepare_dataset_from_feature_file(features_path):
    # hello garbage collector!
    features = load_features(features_path)
    feature_slice = features.root.track[:]
    X = feature_slice['spectrogram']
    Y = feature_slice['target'].astype(numpy.int16)

    logging.info("Preprocessing data...")
    preprocessing.StandardScaler(copy=False).fit_transform(X, Y)  # we can save the scaler

    logging.debug("Shuffling...")
    shuffle_in_unison(X, Y)

    logging.debug("Splitting...")
    (train_x, valid_x, test_x), (train_y, valid_y, test_y) = split_dataset(X, Y, train_valid_test_ratios)
...
if __name__ == '__main__':
    ...
    classifier = LogisticRegression(input=x, n_in=train_set_x.get_value(borrow=True).shape[1] * train_set_x.get_value(borrow=True).shape[2], n_out=train_set_y.get_value(borrow=True).shape[1])
